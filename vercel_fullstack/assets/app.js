...
function bigHolderRow(r){
  return `<tr><td>${esc(r.category||'N/D')}</td><td><span class="operator">${esc(r.operator||'N/D')}</span>${r.note?`<span class="sub">${esc(r.note)}</span>`:''}</td><td class="num">${btcChangeHtml(r.currentBtc,r.previousBtc)}</td><td class="num">${positionCell(r.currentBtc,r.currentDate)}</td><td class="num">${positionCell(r.previousBtc,r.previousDate)}</td><td class="num">${previousChangeHtml(r.previousBtc,r.thirdBtc)}</td><td class="num">${positionCell(r.thirdBtc,r.thirdDate)}</td><td>${sourceCell(r.sourceUrl || '#', r.source || 'Fonte')}</td></tr>`;
}
function bigHolderRows(data){
  const rows=Array.isArray(data?.rows) ? data.rows : [];
  if(!rows.length) return unavailableRows();
  return rows.map(bigHolderRow).join('');
}
...
async function renderOverview(config){
  ...
  const bigHolders=snap?.data?.bigHolders || await safeProvider(config,'/api/big-holders-summary',{ok:false,rows:[]});
  ...
  setHtml('overview-watch-body',bigHolderRows(bigHolders));
}
...
async function renderMarket(config){
  ...
  setHtml('market-watch',bigHolderRows(bigHolders));
}
