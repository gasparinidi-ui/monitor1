...
function cleanNum(input){
  ...
  const txt=String(input).replace(/[$,]/g,'').replace(/\s+/g,'').trim();
...
export default async function handler(req,res){
  ...
  try{
    ...
    $('table tbody tr').each((_,tr)=>{
      ...
      if(cells.length>=4){rows.push({company:cells[0]||null,ticker:cells[1]||null,btcHeld:cleanNum(cells[2]),valueUsd:cleanNum(cells[3])});}
    });
